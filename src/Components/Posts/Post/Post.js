...
function Post({ postId, user, username, caption, imageUrl, noLikes, userId }) {
  const [comments, setComments] = useState([]);
  const [comment, setComment] = useState("");
  const [show, setShow] = useState("");
  const [textShow, setTextShow] = useState("like2");
  const [posterImage, setPosterImage] = useState("textforlike");
    ...
    db.collection("posts")
      ...
      .then((doc2) => {
        if (doc2.data()) {
          if (show === "like2") {
            setShow("like2 blue");
            setTextShow("textforlike bluetextforlike");
          } else {
            setShow("like2");
            setTextShow("textforlike");
          }
        }
      });
