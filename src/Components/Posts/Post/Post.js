...
function Post({
  ...
  const [show, setShow] = useState("likeIcon");
  const [textShow, setTextShow] = useState("likeText");

  // get all the users having posts
  useEffect(() => {
    if (userId) {
      db.collection("users")
        .doc(userId)
  ...
  useEffect(() => {
    let unsubscribe;
    if (postId) {
      // get all the comments with time order
      unsubscribe = db
        .collection("posts")
        .doc(postId)
        .collection("comments")
  ...
  useEffect(() => {
    db.collection("posts")
      ...
      .then((doc2) => {
        if (doc2.data()) {
          if (show === "likeIcon") {
            setShow("likeIcon blue");
            setTextShow("likeText blueLikeText");
          } else {
            setShow("likeIcon");
            setTextShow("likeText");
          }
        }
      });
    ...
    db.collection("posts")
      ...
      .then((doc1) => {
        ...
        if (show === "likeIcon") {
          db.collection("posts")
            ...
            .then((doc2) => {
              if (doc2.data()) {
                ...
                // only set likes to 1 because user can only click like 1 time
                // second time user click it we remove like for the post
                db.collection("posts")
                  .doc(postId)
                  .collection("likes")
                  .doc(user.uid)
                ...
                // update number of likes of the post to database
                db.collection("posts")
                  .doc(postId)
                  .update({
          ...
          // if user remove like for the post
          db.collection("posts")
            .doc(postId)
            .collection("likes")
            .doc(user.uid)
  ...
  return (
    <div className="post">
      ...
      <div className="post__hr" />

      <div className="post__reactions">
        <div className="like" onClick={likeHandle}>
          <i className={show} />
          <h3 className={textShow}>Like</h3>
        </div>
        ...
        <div className="share">
          ...
          <h3 className="shareText">Share</h3>
        </div>
      </div>
      ...
      {comments.length > 0 &&
        comments.map((comment) => (
          ...
          >
            <Avatar
              className="post__avatar2__comment"
              alt=""
