...
function Header({ user, selected }) {
  ...
  // users left after filtered by search
  ...
  const collapseSearchBar = () => {
    ...
    document.getElementsByClassName("header__searchBack")[0].style.display =
      "none";
    document.getElementsByClassName("header__search")[0].style.display =
      "block";
    document.getElementsByClassName("searchBar")[0].style.display = "none";
    document.getElementsByClassName("dropdown__result")[0].style.display =
      "none";
  ...
  // re-render everytime search term input changes
  useEffect(() => {
  ...
  const renderNotifications = () => {
    if (isNotificationOpen) {
      ...
      document
        .getElementsByClassName("dropdown__notifications")[0]
        .classList.remove("block");
    } else {
      setNotificationsOpen(true);
      setProfileDown(false);
      document
        .getElementsByClassName("dropdown__options")[0]
        .classList.remove("block");
      document
        .getElementsByClassName("dropdown__notifications")[0]
        .classList.add("block");
  ...
  useEffect(() => {
    db.collection("posts").onSnapshot((snapshot1) => {
      // eslint-disable-next-line array-callback-return
      snapshot1.docs.map((doc1) => {
        // only get posts of current user
        if (user && doc1.data().username === user.displayName) {
          db.collection("posts")
            .doc(doc1.id)
            .collection("comments")
  ...
  return (
    <div className="header">
      <div className="logo__searchbar">
        ...
        <div className="header__searchBack" onClick={collapseSearchBar}>
          <i className="searchBackIcon" />
        </div>

        <div className="header__search" onClick={expandSearchBar}>
      ...
      <div className="header__middle__icons">
        <div className="homeIcon">
          {selected ? (
            ...
            <Link to="/">
              <svg viewBox="0 0 28 28" className="icon" height="28" width="28">
      ...
      <div className="header__end__icons">
        ...
        <div className="round" onClick={renderNotifications}>
          <svg
            viewBox="0 0 28 28"
            alt=""
            className={`notificationsIcon ${isNotificationOpen && "blue"}`}
            height="20"
            width="20"
          >
        ...
        <div className="dropdown__notifications">
          ...
          {notifications.length === 0 ? (
            <div className="noNotifications">
              <h1 className="noNotificationsText">
